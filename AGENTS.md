...
Guidance for AI coding agents (Codex, Gemini CLI, etc.) working in this repository.
...
- `frontend/analyzer.js` — **auto-generated**; do not edit by hand (see below)
...
The page auto-detects its backend: if `window.SKILLXRAY_API_BASE` is set in `config.js`, it uses that; otherwise local HTTP dev uses `http://localhost:8080`. The public site shows a setup notice and disables scans until a backend is configured.
...
The builder reads `_manifest64_summary.json` and per-skill `step4_harm.json` files, asserts move/violation counts match the manifest, strips absolute machine paths, and writes `findings-data.json`. Serve the frontend over HTTP to test the snapshot (the page fetches it via HTTP).
...
For a deployed backend: set `window.SKILLXRAY_API_BASE` in `frontend/config.js` and add the Pages URL to the backend's `SKILLXRAY_CORS_ORIGINS`. If the Pages workflow fails on `configure-pages` with a permissions error: Settings → Pages → Source: **GitHub Actions**.

## Key design constraints

- API keys are never saved to browser storage; cleared after acceptance and on provider switch.
